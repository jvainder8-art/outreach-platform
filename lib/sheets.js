...
function buildOAuthClient(authConfig) {
  const keys = process.env[authConfig.keysEnvVar]
    ? JSON.parse(process.env[authConfig.keysEnvVar])
    : JSON.parse(fs.readFileSync(resolvePath(authConfig.keysPath)));

  const creds = process.env[authConfig.credsEnvVar]
    ? JSON.parse(process.env[authConfig.credsEnvVar])
    : JSON.parse(fs.readFileSync(resolvePath(authConfig.credsPath)));
