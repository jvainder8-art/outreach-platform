...
async function sendViaOAuth(oauthClient, gmailAuth, to, subject, body) {
  const gmail = google.gmail({ version: 'v1', auth: oauthClient });
  const fromLine = (gmailAuth.displayName && gmailAuth.email)
    ? `From: "${gmailAuth.displayName}" <${gmailAuth.email}>\n`
    : '';
...
async function send(clientConfig, oauthClient, to, subject, body, attachmentPath) {
  ...
  if (gmailAuth.method === 'oauth') {
    await sendViaOAuth(oauthClient, gmailAuth, to, subject, body);
  } else {
