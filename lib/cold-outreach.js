...
async function run(clientConfig) {
  ...
  for (let i = 1; i < rows.length && contacts.length < Math.min(limit, remaining); i++) {
    ...
    if (status && status !== 'no status' && status !== 'new') continue;
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    try {
      ...
      const fallbackLine = 'I came across your brand and was genuinely drawn to what you are building.';
      let personalLine = fallbackLine;
      if (brandInfo) {
        const prompt = `Write ONE short sentence (under 20 words) that fits naturally in a cold outreach email from ${clientConfig.persona.bio}. Reference something specific and genuine about this brand. No emojis, no dashes, no exclamation points. Just the sentence.\n\nBrand content:\n${brandInfo}`;
        try {
          personalLine = await generate(clientConfig.ai, null, prompt);
        } catch (aiErr) {
          log(name, `AI generation failed for ${contact.email} (${aiErr.message}), using fallback line.`);
        }
      }
