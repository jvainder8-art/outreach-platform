...
const { getRows, updateRow, getStyleGuide } = require('./sheets');
...
async function run(clientConfig, clientDir) {
  ...
  const baseSystemPrompt = loadSystemPrompt(clientDir);
  const styleGuide = await getStyleGuide(clientConfig.auth.sheets, clientConfig.sheetId);
  const styleGuideBlock = Object.keys(styleGuide).length > 0
    ? '\n\n## Voice & Style Guide\n' + Object.entries(styleGuide).map(([k, v]) => `${k}: ${v}`).join('\n')
    : '';
  const systemPrompt = (baseSystemPrompt || '') + styleGuideBlock;
